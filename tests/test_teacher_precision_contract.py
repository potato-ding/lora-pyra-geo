...
def test_contract_violation_is_fatal_and_machine_readable():
    with pytest.raises(RuntimeError) as exc_info:
        require_contract_dtype("teacher_forward_input", torch.float16, "teacher_input")
    ...
    assert "expected=bfloat16" in message
    assert "actual=float16" in message
    assert "experiment_aborted=True" in message


def test_precision_log_fields_describe_bf16_compute_and_fp32_retrieval():
    fields = precision_contract_log_fields()
    assert fields == {
        "precision_contract": "T0-v1.0",
        "precision_mode": "bf16_backbone_fp32_retrieval",
        "teacher_compute_dtype": "bfloat16",
