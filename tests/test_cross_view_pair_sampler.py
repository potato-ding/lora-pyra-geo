...
try:
    ...
    fake_transforms.get_paired_cross_view_train_transforms = lambda *args, **kwargs: (None, None)
...
class FakePairedCrossViewDataset:
    def __init__(self, pair_pids):
        self.pair_pids = pair_pids


class CrossViewPairSamplerTest(unittest.TestCase):
    def test_batches_have_unique_pids_and_pairs_are_not_reused(self):
        pair_pids = []
        for pid in range(6):
            pair_pids.extend([f"{pid:04d}"] * 3)

        dataset = FakePairedCrossViewDataset(pair_pids)
        sampler = CrossViewPairSampler(dataset, batch_size=4, seed=7)
        sampler.set_epoch(2)
    ...
    def test_rejects_batch_larger_than_pid_count(self):
        dataset = FakePairedCrossViewDataset(["0001", "0001", "0002"])

        with self.assertRaises(ValueError):
            CrossViewPairSampler(dataset, batch_size=3)
    ...
    def test_distributed_ranks_receive_disjoint_slices_of_pid_unique_global_batches(self):
        ...
        try:
            ...
            rank0_sampler = CrossViewPairSampler(dataset, batch_size=2, seed=17)
            rank0_sampler.set_epoch(3)
            rank0_batches = list(rank0_sampler)

            teacher_datasets.dist = FakeDist(rank=1)
            rank1_sampler = CrossViewPairSampler(dataset, batch_size=2, seed=17)
