...
require_inputs
export CUDA_VISIBLE_DEVICES="${P1_GPUS:-2,3}"
MASTER_PORT="$MASTER_PORT_P1"
check_master_port P1 "$MASTER_ADDR" "$MASTER_PORT"
print_distributed_launch P1 "$MASTER_ADDR" "$MASTER_PORT" 2
run_cmd "$PYTHON_BIN" -m torch.distributed.run \
  --master_addr "$MASTER_ADDR" --master_port "$MASTER_PORT" --nproc_per_node=2 \
  -m src.diagnostics.train_frozen_probe --probe P1 --deepspeed \
