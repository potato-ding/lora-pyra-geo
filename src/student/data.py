...
class U1652PairDataset(Dataset):
    ...
    def _collect_images(directory):
        return [os.path.join(directory, name) for name in sorted(os.listdir(directory)) if name.lower().endswith(('.jpg', '.jpeg', '.png'))]
...
def create_student_train_dataset_and_loader(args):
    ...
    return DataLoader(dataset=train_dataset, batch_size=args.batch_size, shuffle=False, num_workers=getattr(args, 'num_workers', 8), pin_memory=getattr(args, 'pin_memory', True), drop_last=False)
