...
import sys
import socket
from .spatial_final import load_config
...
def main(argv=None):
    ...
    if args.smoke_output:cfg=dict(cfg,output_dir=args.smoke_output,sealed_provenance_file=None)
    ...
    command=[sys.executable,'-m','src.student.train_spatial_final','--config',str(Path(args.config).resolve())]
    if args.smoke_output:command.extend(["--smoke-output",args.smoke_output])
