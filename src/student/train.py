...
from .dual_stst import DualSTSTSupervision, deployment_state_dict, file_sha256, stst_total_loss
...
def batch_loss(engine,teacher,images,local_pairs,criterion,cfg,epoch):
    ...
    if cfg['mode']=='baseline':return info,{'infonce':info.detach(),'dual_stst':info.detach()*0}
    with torch.no_grad():
...
def main():
    ...
    ds={'train_batch_size':32,'train_micro_batch_size_per_gpu':16,'gradient_accumulation_steps':1,
    ...
    criterion=PairInfoNCE(label_smoothing=cfg['label_smoothing']);best=float('-inf')
    if dist.get_rank()==0:
    ...
    for epoch in range(1,cfg['epochs']+1):
        ...
        for step,batch in enumerate(train_loader):
            ...
            if dist.get_rank()==0 and step%200==0:
                print(json.dumps({'epoch':epoch,'step':step,'loss':float(loss.detach()),**{k:float(v) for k,v in components.items()}}),flush=True)
        engine.eval();metrics={}
        ...
        if dist.get_rank()==0:
            ...
            with (output/'epoch_metrics.jsonl').open('a') as f:f.write(json.dumps({'epoch':epoch,'metrics':metrics})+'\n')
