"""Opt-in Group-I entry, retaining the canonical Part-II runtime and selector."""
import argparse
import json
import os
...
from .spatial_group1 import load_config, parent_config, Group1TrainingModel, batch_loss, pretrained_check, gradient_check
def main():
    ...
    if cfg.get('top_interface', 'linear') != 'linear':
        ...
        prepare_top(supervision, parent_config())
    # P2_INTEGRATION_END
    model=Group1TrainingModel(student,supervision,cfg['spatial_objective']).to(device)
    ...
    if dist.get_rank()==0:
        ...
        run_metadata.update(part='Part-III',research_axis='same_image_spatial_transfer',
            ...
            spatial_projector_storage='float32',spatial_l2_loss='float32',
            no_cross_view_position_matching=True,canonical_student_forward='one_concat_N64',
            point_projector_train_only=True,relational_input='raw_stage3_no_projector')
    ...
    for epoch in range(1,(1 if cli.smoke_output else cfg['epochs'])+1):
        ...
        for step,batch in enumerate(train_loader):
            ...
            # BNCC_END
            if cli.smoke_output:
                spatial_grad=torch.autograd.grad(engine.module.spatial_objective,
                    tuple(engine.module.student.backbone.features[37].parameters()),retain_graph=True)
                assert spatial_grad and all(torch.isfinite(g).all() for g in spatial_grad)
                assert sum(float(g.float().norm()) for g in spatial_grad)>0
            engine.backward(loss)
            if step%200==0:
                components['spatial_head_grad_finite']=gradient_check(engine);grad_checks+=1
            engine.step()
            engine.module.spatial_objective=None
            epoch_steps+=1
        ...
        print('SPATIAL_EPOCH='+json.dumps(dict(epoch=epoch,steps=epoch_steps,
            **{k:v/epoch_steps for k,v in epoch_sums.items() if k!='spatial_head_grad_finite'},
            spatial_head_grad_finite=True,gradient_checks=grad_checks)),flush=True)
        # BNCC_BEGIN
        if cfg.get('bncc_enabled', False):
        ...
        if cli.smoke_output:
            ...
            write_json(output/'SMOKE_PASS.json',dict(pass_status=True,canonical_N64=True,
                ...
                spatial_objective=cfg['spatial_objective']))
