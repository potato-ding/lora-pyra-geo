...
def main(allow_kd=True,allow_abv=False):
    ...
    from src.middle_teacher.froute_support import validate_froute,component_name,runtime_for,fingerprints
    validate_froute(config,validate_r0)
    component=component_name(config)
    allow_abv=component=='adaptive_bridge_v2'
    assert world_size()==2
    ...
    if not args.smoke_no_step:
        ...
        barrier()
        assert Path(os.environ['FROUTE_EXTERNAL_TRAIN_LOG']).resolve()==(output/'train.log').resolve()
    if allow_abv:
    ...
    kd=None
    if config['distillation'][component]['enabled']:
        runtime_class=runtime_for(component)
