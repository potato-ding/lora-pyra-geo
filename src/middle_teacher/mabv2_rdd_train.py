...
def main(allow_kd=True,allow_abv=False):
    ...
    from src.middle_teacher.mabv2_rdd_runtime import validate_sweep,MABV2RDDRuntime,fingerprints
    validate_sweep(config,validate_r0)
    ...
    if not args.smoke_no_step:
        ...
        barrier()
        assert Path(os.environ['MABV2_RDD_EXTERNAL_TRAIN_LOG']).resolve()==(output/'train.log').resolve()
    if allow_abv:
    ...
    for epoch in range(1,11):
        ...
        for batch_index,(drone,satellite,labels,pids) in enumerate(loader):
            ...
            if args.smoke_no_step:
                ...
                def record_gradient(grad):
                    ...
                    gradient_squared_norms.append(float(grad.detach().float().square().sum()))
            ...
            if args.smoke_no_step:
                ...
                kd_stats['middle_total_local_backward_hook_gradient_norm']=math.sqrt(sum(gradient_squared_norms))
                kd_stats['gradient_norm_semantics']='local autograd hook gradients during DeepSpeed backward; before clipping and optimizer step'
            ...
            if kd is not None:
                for key,value in dict(kd_stats,base_loss=float(base_loss.detach())).items():
                    if key.endswith(('_loss','_effective_weight','_to_infonce_ratio')) or key in ('abv_gate_alpha_28','abv_gate_alpha_36','abv_gate_entropy'):component_sums[key]=component_sums.get(key,0.)+value
        ...
        if rank()==0:
            ...
            row['rdd_weighted_over_infonce_mean_loss']=row['loss_components']['retrieval_distribution_kd_weighted_loss']/max(row['loss_components']['base_loss'],1e-12)
            rows.append(row)
