...
def build_arg_parser():
    ...
    parser.add_argument('--training_stage', type=str, choices=['auto', 'sample4geo', 'identity', 'hard_pool'], default='auto', help='Explicit staged-restart mode; auto keeps legacy curriculum switches')
    parser.add_argument('--init_checkpoint', type=str, default=None, help='Load teacher trainable weights before starting this training stage')
    parser.add_argument('--init_checkpoint_strict_trainable', type=str2bool, nargs='?', const=True, default=True, help='Require init checkpoint to cover all current trainable teacher parameters')
    ...
    parser.add_argument('--build_hard_pool_before_train', action='store_true', help='Build hard_pool once before epoch 1, useful for hard-only restart training')
