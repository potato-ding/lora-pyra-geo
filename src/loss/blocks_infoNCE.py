    ...
    def forward(self, query_features, reference_features, logit_scale):
        ...
        if self.last_runtime_audit is None:
            self.last_runtime_audit = {
                ...
                "d2s_loss_dtype": loss_q2r.dtype,
                "s2d_loss_dtype": loss_r2q.dtype,
                "logits_nan": int(torch.isnan(logits.detach()).sum().item()),
                "logits_inf": int(torch.isinf(logits.detach()).sum().item()),
                "d2s_loss_nan": int(torch.isnan(loss_q2r.detach()).sum().item()),
                "d2s_loss_inf": int(torch.isinf(loss_q2r.detach()).sum().item()),
                "s2d_loss_nan": int(torch.isnan(loss_r2q.detach()).sum().item()),
                "s2d_loss_inf": int(torch.isinf(loss_r2q.detach()).sum().item()),
            }
